...
| Fresh sessions | Use `/new` to request Pi's official same-session replacement in the current Telegram thread when the host capability is installed. | A fresh session preserves the exact Telegram target without TUI or subprocess tricks. |
